...
function getLanguageName(code) {
    ...
    return languageMap[code] || code; // Fallback to the code if not in map
}
 ...
 */
// REPLACEMENT: A much higher-quality translation using an LLM
async function translateWithLlama(text, targetLang, env) {
    const targetLanguageName = getLanguageName(targetLang);

    // This detailed prompt is the key to high-quality, nuanced translation.
    const prompt = `
...
${targetLanguageName} Translation:`;
    ...
    try {
        const aiResponse = await env.AI.run('@cf/meta/llama-3-8b-instruct', {
            prompt: prompt,
            max_tokens: 500 // Give enough tokens for the translation
        });
