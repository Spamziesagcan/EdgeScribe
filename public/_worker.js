...
async function translateWithProtectedWords(text, targetLang, env) {
  const PROTECTED_WORDS = [
    'Harsh', 'Rocky', 'Lucky', 'Honey', 'Deep', 'Rose', 'Sunny', 'Jasmine', 'Crystal', 'Bill', 'Frank', 'Mark',
    'Amber', 'Brandy', 'Brooks', 'Clay', 'Cliff', 'Dean', 'Drew', 'Duke', 'Forrest', 'Grant', 'Hunter', 'Lance',
    'Miles', 'Reed', 'Rob', 'Roman', 'Rusty', 'Sky', 'Stone', 'Wade', 'Woody', 'Blaze', 'Chase', 'Chip', 'Colt',
    'Dash', 'Jett', 'Link', 'Cash', 'King', 'Legend', 'Major', 'Reign', 'Royal', 'Saint', 'Wilder', 'Zen', 'Angel',
    'Blue', 'Cricket', 'Destiny', 'Faith', 'Grace', 'Harmony', 'Haven', 'Heaven', 'Honor', 'Hope', 'Journey', 'Joy',
    'Justice', 'Liberty', 'Melody', 'Mercy', 'Patience', 'Peace', 'Precious', 'Serenity', 'Trinity', 'True',
    'Wisdom', 'Winter', 'August', 'Christian', 'Genesis', 'Noel', 'Paris', 'Reagan', 'Zion', 'Boat', 'Apple',
...
async function summarizeInChunks(text, env) {
  ...
  const summaryPromises = chunks.map(chunk =>
    env.AI.run("@cf/facebook/bart-large-cnn", {
      // CHANGE 1: Wrap input in a 'requests' array
      requests: [{
        input_text: chunk,
        max_length: 150,
      }]
  ...
  const combinedSummaries = chunkSummaryResponses
    // CHANGE 2: Filter and access the first element of the response array
  ...
  try {
    const finalSummaryResponse = await env.AI.run("@cf/facebook/bart-large-cnn", {
      // CHANGE 3: Wrap the final input as well
      requests: [{
    ...
    // CHANGE 4: Access the first element of the final response
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        // If input is shorter than 100 characters, skip summarization
        // Step 2: Summarization logic

        if (englishText.length < 200) { // No need to chunk for very short text
          ...
          try {
            const summaryResponse = await env.AI.run("@cf/facebook/bart-large-cnn", {
              // CORRECTED: Input is wrapped in a 'requests' array
              requests: [{
                input_text: englishText,
                max_length: 100,
              }]
            });
            // CORRECTED: Access the summary from the first element of the response array
            ...
            summaryText = englishText; // Fallback to original text on error
          }
        } else {
          // This calls the separate `summarizeInChunks` function which you already updated
        ...
        // This deduplication function remains the same and should be kept
        ...
        // Translate summary or original text with protected words
        if (targetLang !== 'en') {
