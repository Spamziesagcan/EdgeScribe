// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - WITH POST-PROCESSING)
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        const translationPrompt = `Translate to ${targetLang} but keep proper nouns in English: "${summaryText}"`;
        ...
        // --- FINAL FIX: CLEAN THE AI's RESPONSE ---
        // If the response contains a colon, we assume the AI included our instructions
        // and the real translation is the last part.
        if (translatedText.includes(':')) {
            translatedText = translatedText.split(':').pop().trim();
        }

        // Also clean up any surrounding quote marks.
