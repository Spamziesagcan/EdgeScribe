// _worker.js - For Cloudflare Pages Full-Stack Deployment

// =================================================================================
// HELPER FUNCTIONS AND CONFIGURATION (No changes needed here)
// =================================================================================

// Supported language codes for M2M100 model (subset for validation)
...
function validateInput(text, targetLang) {
  ...
  if (!targetLang || typeof targetLang !== "string") {
    throw new ValidationError("Target language parameter is required and must be a string");
  }
...
// Sanitize cache key
function sanitizeCacheKey(targetLang, text) {
  const textHash = hashString(text.substring(0, 100));
  return `translate:${targetLang}:${textHash}`;
}
...
export default {
  async fetch(request, env, context) {
    ...
    // This is the key change. We check if the request is for an API endpoint.
    // All other requests (like for /main_page.css, /main_page.js, etc.) will be ignored
    // by this function and passed to the static asset server.
    const isApiRequest = url.pathname === '/' || url.pathname.startsWith('/health') || url.pathname.startsWith('/debug');

    if (!isApiRequest) {
      // This is not an API request, so we pass it to the next function
      // which is Cloudflare Pages' static asset server.
      return context.next();
    }
