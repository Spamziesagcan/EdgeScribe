// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - ROBUST CLEANUP)
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        // --- DEFINITIVE FIX: CLEAN THE AI's RESPONSE ---
        if (translatedText.includes(':')) {
            translatedText = translatedText.split(':').pop().trim();
        }

        // This new line is more robust and removes any leading or trailing quotes.
