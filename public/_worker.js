// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - ROBUST PLACEHOLDER METHOD)
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        // --- DEFINITIVE "PLACEHOLDER" METHOD WITH ROBUST PLACEHOLDERS AND EXPANDED LIST ---
        ...
        const sanitizedSummary = summaryText.replace(protectedWordsRegex, (match) => {
            // Using a standard, robust placeholder format like {0}
        ...
        // 3. REPLACE BACK the placeholders with the original English words
        for (const [placeholder, originalWord] of placeholders.entries()) {
            // Must escape the curly braces for the RegExp to work correctly
            const placeholderRegex = new RegExp(placeholder.replace('{', '\\{').replace('}', '\\}'), 'g');
