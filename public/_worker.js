// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - DYNAMIC NER + PLACEHOLDER METHOD)
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        // --- DEFINITIVE "DYNAMIC NER + PLACEHOLDER" METHOD ---

        // 1. DYNAMICALLY FIND NAMES AND BRANDS using a robust NER model.
        const nerResponse = await env.AI.run("@cf/dbmdz/bert-large-cased-finetuned-conll03-english", { text: summaryText });
        
        const placeholders = new Map();
        let placeholderIndex = 0;
        let sanitizedSummary = summaryText;

        if (Array.isArray(nerResponse) && nerResponse.length > 0) {
            const entitiesToProtect = nerResponse
                .filter(entity => entity.entity_group === 'PER' || entity.entity_group === 'ORG') // Protect Persons and Organizations
                .map(entity => entity.word);
            
            const uniqueEntities = [...new Set(entitiesToProtect)];

            if (uniqueEntities.length > 0) {
                const protectedWordsRegex = new RegExp(`\\b(${uniqueEntities.join('|')})\\b`, 'gi');
                sanitizedSummary = summaryText.replace(protectedWordsRegex, (match) => {
        ...
        // 2. TRANSLATE the sanitized text
        let translatedText = sanitizedSummary; // Default to sanitized text
        if (targetLang !== 'en') {
            ...
            if (!translationResponse?.translated_text) { throw new AIServiceError("Translation failed."); }
            translatedText = translationResponse.translated_text;
        }

        // 3. REPLACE BACK the placeholders with the original English names/brands
        ...
        const result = JSON.stringify({
          summary: summaryText, // The original English summary
          translated: translatedText, // The perfectly translated text
