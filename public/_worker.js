...
if (englishText.length < 100) {
  ...
  // Post-processing: ensure at least 5 sentences for longer passages
  function countSentences(text) {
    return text.split(/[.!?]+/).filter(s => s.trim().length > 0).length;
  }

  if (englishText.length > 400 && countSentences(summaryText) < 5) {
    const sentences = englishText.match(/[^\.!\?]+[\.!\?]+/g) || [];
    summaryText += " " + sentences.slice(0, 5 - countSentences(summaryText)).join(" ");
  }
