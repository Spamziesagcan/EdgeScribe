    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        if (targetLang !== 'en') {
            ...
            const textFragments = summaryText.split(protectedWordsRegex);
            const namesFound = [...summaryText.matchAll(protectedWordsRegex)].map(match => match[0]);

            const translationPromises = textFragments.map(fragment => {
                // Don't translate empty strings, just return them
                if (!fragment || fragment.trim() === '') {
                    return Promise.resolve({ translated_text: fragment });
                }
            ...
            const translatedFragments = translatedResponses.map(resp => resp.translated_text);

            // Stitch the translated fragments and the original names back together
            let finalResult = '';
            for (let i = 0; i < translatedFragments.length; i++) {
                finalResult += translatedFragments[i];
                if (i < namesFound.length) {
                    finalResult += namesFound[i];
                }
            }
