// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - DUAL LANGUAGE)
...
function validateInput(text, sourceLang, targetLang) {
    ...
    if (!sourceLang || typeof sourceLang !== "string" || !SUPPORTED_LANGUAGES.includes(sourceLang)) { throw new ValidationError(`Invalid source language provided.`); }
    if (!targetLang || typeof targetLang !== "string" || !SUPPORTED_LANGUAGES.includes(targetLang)) { throw new ValidationError(`Invalid target language provided.`); }
}
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        if (sourceLang !== 'en') {
            ...
            if (!toEnglishResponse?.translated_text) {
                throw new AIServiceError("Failed to translate source text to English.");
            }
            englishText = toEnglishResponse.translated_text;
        }

        // Now, summarize the English text.
        ...
        const result = JSON.stringify({
          summary: summaryText, // The English summary
