// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - CURATED LIST + PLACEHOLDER METHOD)
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        // --- DEFINITIVE "PLACEHOLDER" METHOD WITH A CURATED LIST ---
        
        // 1. FIND & REPLACE WITH PLACEHOLDERS
        const PROTECTED_NAMES = [ 'Harsh', 'Rocky', 'Lucky', 'Honey', 'Deep', 'Rose', 'Sunny', 'Jasmine', 'Crystal', 'Bill', 'Frank', 'Mark', 'Amber', 'Brandy', 'Brooks', 'Clay', 'Cliff', 'Dean', 'Drew', 'Duke', 'Forrest', 'Grant', 'Hunter', 'Lance', 'Miles', 'Reed', 'Rob', 'Roman', 'Rusty', 'Sky', 'Stone', 'Wade', 'Woody', 'Blaze', 'Chase', 'Chip', 'Colt', 'Dash', 'Jett', 'Link', 'Cash', 'King', 'Legend', 'Major', 'Reign', 'Royal', 'Saint', 'Wilder', 'Zen', 'Angel', 'Blue', 'Cricket', 'Destiny', 'Faith', 'Grace', 'Harmony', 'Haven', 'Heaven', 'Honor', 'Hope', 'Journey', 'Joy', 'Justice', 'Liberty', 'Melody', 'Mercy', 'Patience', 'Peace', 'Precious', 'Serenity', 'Trinity', 'True', 'Wisdom', 'Winter', 'August', 'Christian', 'Genesis', 'Noel', 'Paris', 'Reagan', 'Zion' ];
        const protectedWordsRegex = new RegExp(`\\b(${PROTECTED_NAMES.join('|')})\\b`, 'gi');
        
        const placeholders = new Map();
        let placeholderIndex = 0;

        const sanitizedSummary = summaryText.replace(protectedWordsRegex, (match) => {
        ...
        let translatedText = sanitizedSummary;
        if (targetLang !== 'en') {
        ...
        // 3. REPLACE BACK the placeholders with the original English names
