// _worker.js - For Cloudflare Pages Full-Stack Deployment (DEFINITIVE - FLEXIBLE PLACEHOLDER REPLACEMENT)
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/") {
        ...
        if (targetLang !== 'en') {
            const translationResponse = await env.AI.run("@cf/meta/m2m100-1.2b", { text: sanitizedSummary, source_lang: "en", target_lang: targetLang });
        ...
        // 3. REPLACE BACK with a flexible Regex to handle spaces
        for (const [placeholder, originalWord] of placeholders.entries()) {
            const numberInside = placeholder.replace(/[{}]/g, '');
            // This regex looks for `{`, optional spaces, the number, optional spaces, and `}`
            const placeholderRegex = new RegExp(`\\{\\s*${numberInside}\\s*\\}`, 'g');
